...
maybeGet =
  \case
    ...
    Nothing ->
      error
        "Panic! Error! This method shouldn't been used in context where Maybe contains Nothing"
...
byNcmpResult x y resEq resLt fnGt =
  case ncmp x y of
    EQ -> resEq
    LT -> resLt
    GT -> fnGt $ maybeGet $ nsub x y -- unsafe, but other cases exclude Nothing from nsub

ndiv :: N -> N -> N
-- In my opinion it would be better if it return Maybe N, because it is not clear what to return, except throw error,
-- in case second argument is Z
