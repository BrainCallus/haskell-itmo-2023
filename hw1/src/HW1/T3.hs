...
handleLeftRotate tree =
  case tree of
    Leaf -> Leaf --this point will never achieved because we call method only if balance=2 that leafs can never satisfy,
  -- but this exhaustive condition a simple way to avoid warnings
...
handleRightRotate =
  \case
    Leaf -> Leaf --this point will never achieved
    (Branch m left v right) ->
